...
const DEFAULT_CRON = "0 7,12 * * 1-5";
const DEFAULT_CRON_SECONDARY = "45 15 * * 1-5";
...
function nextScheduledRunIso(now = new Date()) {
  ...
  const makeUtcDate = (year, month, day, hour, minute) => {
    // Approximate NY-local target by searching nearby UTC times that format to the desired NY wall-clock time.
  ...
  const weekdayIndex = weekdayNames.indexOf(String(p.weekdayText || "").toLowerCase());
  for (let dayOffset = 0; dayOffset < 10; dayOffset += 1) {
    ...
    for (const slot of DEFAULT_SCAN_SLOTS_NY) {
      const target = makeUtcDate(parts.year, parts.month, parts.day, slot.hour, slot.minute);
...
function formatMarketAlert(scan, rows) {
  ...
  const biasEmoji = String(scan.marketBias || "").toLowerCase().includes("bull") ? "🟢" : String(scan.marketBias || "").toLowerCase().includes("bear") ? "🔴" : "🟡";
  const actionEmoji = (action) => {
    const value = String(action || "").toLowerCase();
    if (value.includes("long") || value.includes("buy") || value.includes("trade")) return "🟢";
    if (value.includes("short") || value.includes("sell")) return "🔴";
    if (value.includes("watch") || value.includes("wait")) return "🟡";
    return "⚪";
  };
  const header = [
    `${biasEmoji} ADOL22 Market Scan`,
    ...
    `🧮 Trade: ${ms.trade || 0} | Wait: ${ms.wait || 0} | Bulls: ${ms.bulls || 0} | Bears: ${ms.bears || 0}`
  ].join("\n");
  const bullLines = topBulls.map((s, i) => `${i + 1}. ${s.ticker} $${round(s.price,2)} | Bull ${s.bullScore} | Final ${s.finalAiScore} | RVOL ${s.rvol} | ${actionEmoji(s.action)} ${s.action}`).join("\n");
  const bearLines = topBears.map((s, i) => `${i + 1}. ${s.ticker} $${round(s.price,2)} | Bear ${s.bearScore} | Final ${s.finalAiScore} | RVOL ${s.rvol} | ${actionEmoji(s.action)} ${s.action}`).join("\n");
  ...
  return [
    ...
    "",
    "🔵 Liquidity Map / Where The Money Flows",
    `Inflow: ${inflowText || "-"}`,
    `Outflow: ${outflowText || "-"}`,
    "",
    "🟠 Institutional Mode",
...
function createAdolEngine(opts = {}) {
  ...
  function getStatus() {
    ...
    return {
      ...
      scheduler: {
        ...
        running: isRunning,
        cron: `${DEFAULT_CRON} + ${DEFAULT_CRON_SECONDARY}`,
        timezone: DEFAULT_TZ,
        lastRunAt: lastRunAt || "",
  ...
  async function maybeSendAlerts(scan, trigger = "manual") {
    ...
    alerts.reason = "technical_alerts_only";
  ...
  function initCron() {
    ...
    try {
      ...
      cronTask = [
        ...
        opts.cron.schedule(DEFAULT_CRON_SECONDARY, jobRunner, { timezone: DEFAULT_TZ })
      ];
      log(`[ADOL] cron started ${DEFAULT_CRON} + ${DEFAULT_CRON_SECONDARY} ${DEFAULT_TZ}`);
    } catch (error) {
      cronTask = [];
