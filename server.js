...
const ADOL_CRON_EXPRESSION = "*/5 7-20 * * 1-5";
...
function buildTelegramRowLine(row, index) {
  ...
  return `${index + 1}. ${telegramActionEmoji(action)} ${row.ticker} ${formatTickerPrice(row.price)} | ${side} | Final ${finalScore} | Bull ${bull} | Bear ${bear} | RVOL ${rvol}`;
}
...
function buildBuyReply(scan, symbols = []) {
  ...
  const buys = scoped
    ...
    .sort((a, b) => {
      const tradeWeight = (String(b.action) === "Long") - (String(a.action) === "Long");
      if (tradeWeight !== 0) return tradeWeight;
    ...
    .slice(0, 5);
  if (!buys.length) return "No strong bullish setups are available in the latest ADOL22 scan.";
  return [
    "🟢 Best Buy Setups Now",
...
function buildSellReply(scan, symbols = []) {
  ...
  const sells = scoped
    ...
    .sort((a, b) => {
      const tradeWeight = (String(b.action) === "Short") - (String(a.action) === "Short");
      if (tradeWeight !== 0) return tradeWeight;
      const finalDiff = Number(b.finalAiScore || 0) - Number(a.finalAiScore || 0);
      if (finalDiff !== 0) return finalDiff;
    ...
    .slice(0, 5);
  if (!sells.length) return "No strong bearish setups are available in the latest ADOL22 scan.";
  return [
    "🔴 Best Short Setups Now",
    `Session: ${scan.session}`,
    `Market Bias: ${scan.marketBias}`,
    "",
...
function buildStrictCandidateLine(row, index) {
  ...
  return `${index + 1}. ${telegramActionEmoji(side)} ${row.ticker} ${formatTickerPrice(row.price)} | ${side} | Final ${Math.round(Number(row.finalAiScore || 0))} | RVOL ${Number(row.rvol || 0).toFixed(2)} | ${vwapState} | ${liq}`;
}
